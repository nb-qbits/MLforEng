...
os.environ.setdefault("FLASH_ATTENTION_SKIP_CUDA_IMPORT", "1")

# WORKED WITH 6F NOTEBOOK; ENABLED FOR LORA


def hf_login_on_worker(hf_token: str | None) -> None:
    if not hf_token:
        return
    # Make it available via env (some HF code paths read env vars)
    os.environ["HF_TOKEN"] = hf_token
    os.environ["HUGGINGFACE_HUB_TOKEN"] = hf_token

    # Persist token for huggingface_hub to pick up reliably
    try:
        from huggingface_hub import login
        login(token=hf_token, add_to_git_credential=False)
    except Exception as e:
        print(f"[HF] login() failed: {e!r}")


from mlforeng.llm_finetune.utils import (
...
# def get_pretrained_path(model_id: str) -> str:
#    """
#    Return a path that AutoTokenizer/AutoModel can load.

# Supports:
#      * Hugging Face repo IDs (e.g. 'Qwen/Qwen2.5-0.5B', 'gpt2')
#      * Local directories inside the image (e.g. '/opt/app-root/src/models/Qwen2.5-0.5B')

#    S3 mirroring (llama-2-weights style with a 'main' file) is DISABLED here,
#    because our ocpmodel bucket does not follow that layout.
#    """

    # Case 1: it's an existing directory inside the container → just use it
 #   if os.path.isdir(model_id):
 #       print(f"[get_pretrained_path] Using local directory: {model_id}")
 #       return model_id

    # Case 2: otherwise, treat it as a normal HF repo id and let transformers handle it
 #   print(f"[get_pretrained_path] Using model id directly: {model_id}")
 #   return model_id


def get_pretrained_path(model_id: str) -> str:
    """SIMPLE VERSION: Just return the model_id."""
    return model_id  # <-- NO MIRROR LOGIC!
...
def _test_tokenizer(model_name, hf_token=None):
    ...
    try:
        tok = get_tokenizer(model_name=model_name, special_tokens=["<REPR_END>"], hf_token=hf_token)
        _ = tok("hi", return_tensors="pt")
        print(f"[HF] Tokenizer OK for {model_name}")
    except Exception as e:
        print(f"[HF] Tokenizer failed for {model_name}: {type(e).__name__}: {e}")
        raise
    ...
    print(f"Tokenizer OK for: {model_name}")
...
def hf_auth_kwargs(token: str | None) -> dict:
    if not token:
        return {}
...
def training_function(kwargs: dict):
    ...
    print("=" * 50)
    print("DEBUG: training_function started")
    print("=" * 50)
    ...
    # HF Token help
    hf_token = (kwargs.get("args", {}) or {}).get("hf_token") or os.getenv("HF_TOKEN") or os.getenv("HUGGINGFACE_HUB_TOKEN")

    # Make token visible on every Ray worker process
    if hf_token:
        os.environ["HF_TOKEN"] = hf_token
        os.environ["HUGGINGFACE_HUB_TOKEN"] = hf_token

    # Ensure a writable cache on every worker (containers often have read-only $HOME)
    hf_cache_root = os.getenv("HF_HOME") or os.path.join(tempfile.gettempdir(), "hf")
    ...
    os.environ.setdefault("HF_HUB_CACHE", os.path.join(hf_cache_root, "hub"))

    # Make token visible to huggingface_hub on each worker process
    if hf_token:
        os.environ["HF_TOKEN"] = hf_token
        os.environ["HUGGINGFACE_HUB_TOKEN"] = hf_token
    # HF Token help End
    ...
    # print(f"[MODEL] Using mirror URI: {model_id}")
    #  download_path = get_download_path(model_id)

    # Check if model is in S3 or local/HF
    # if model_id.startswith("s3://"):
    #    print(f"[MODEL] Using S3 path directly: {model_id}")
    # Transformers can load from S3 directly, no need for mirror
    # else:
    #    print(f"[MODEL] Using model ID: {model_id} (will use HF or local cache)")
    ...
    # Instead of S3, use Hugging Face directly - HF

    #Start Here - Code to Use HuggingFace Hub Cache Directly

    # hf_model_id = "Qwen/Qwen2.5-0.5B"

    hf_model_id = os.environ["LLM_MODEL_ID"]
    print(f"[MODEL] Loading from HuggingFace Hub: {hf_model_id}")
    print(f"[Token] Token is here before : {hf_token}")

    print(f"[Token] present={bool(hf_token)} len={len(hf_token) if hf_token else 0}")
    hf_login_on_worker(hf_token)
    print("[HF] login done, about to call _test_tokenizer")
    _test_tokenizer(hf_model_id, hf_token=hf_token)
    



    
    # This will use HF cache (~/.cache/huggingface)
    # _test_tokenizer(hf_model_id, hf_token=hf_token)
    print(f"[Token] Token is here : {hf_token}")
    ...
    print(f"[HF] token_present={bool(hf_token)} model_id={model_id}")
    #End Here - Code to Use HuggingFace Hub Cache Directly
   ...
   # _test_tokenizer(args.model_name)
    _test_tokenizer(hf_model_id, hf_token=hf_token)
    tokenizer = get_tokenizer(model_name=hf_model_id,
                              chat_template=chat_template,
                              special_tokens=special_tokens,
                              hf_token=hf_token,
                             )
    ...
    print(f"Loading model (model_id) from {model_id} ...")
    s = time.time()
    model = AutoModelForCausalLM.from_pretrained(
        hf_model_id, 
        trust_remote_code=True,
...
def parse_args():
    ...
    parser.add_argument("--model-name", type=str, default=os.getenv("LLM_MODEL_ID", "gpt2"), 
                        help="Base model (HF repo id or local path)")

    parser.add_argument("--hf-token", type=str, default=os.getenv("HF_TOKEN") or os.getenv("HUGGINGFACE_HUB_TOKEN"), 
                        help="Hugging Face token for gated/private models (or set HF_TOKEN / HUGGINGFACE_HUB_TOKEN).",)
    ...
    parser.add_argument("--num-devices", "-nd", type=int, default=7,
    ...
    parser.add_argument("--ds-config", type=str, default="/opt/app-root/src/MLforEng/mlforeng/llm_finetune/deepspeed_configs/zero_3_offload_optim_param.json",
                        help="Deepspeed config json to use.")    
    
    parser.add_argument("--lora", action="store_true", default=False,
                        help="If passed, will enable parameter efficient fine-tuning with LoRA.")
    
    parser.add_argument("--lora-config", type=str, default="mlforeng/llm_finetune/lora_configs/lora.json",
    ...
    parser.add_argument("--storage-path", type=str, default="/tmp",
...
def main():
  ...
  # ============================================================
    # FIXED: Handle DeepSpeed config - check for base64 first
    # ============================================================
    import os
    import base64
    
    ds_config_value = None
    
    # 1. Check for base64 in environment variable
    ds_config_b64 = os.environ.get("DEEPSPEED_CONFIG_BASE64")
    if ds_config_b64:
        print("ℹ️  Loading DeepSpeed config from base64 environment variable")
        try:
            # Decode base64 string
            config_json = base64.b64decode(ds_config_b64).decode()
            ds_config_value = json.loads(config_json)
            print(f"✅ DeepSpeed config loaded from base64 (ZeRO stage: {ds_config_value.get('zero_optimization', {}).get('stage', 'N/A')})")
        except Exception as e:
            print(f"❌ Failed to decode base64 config: {e}")
            ds_config_value = None
    
    # 2. If no base64, try file path from args
    if ds_config_value is None:
        print(f"ℹ️  Loading DeepSpeed config from file: {args.ds_config}")
        try:
            with open(args.ds_config, "r") as f:
                ds_config_value = json.load(f)
            print(f"✅ DeepSpeed config loaded from file")
        except FileNotFoundError:
            print(f"❌ DeepSpeed config file not found: {args.ds_config}")
            ...
            print("⚠️  Using default DeepSpeed ZeRO-3 config")
    
    # Add deepspeed plugin to the config - NOW PASSING A DICTIONARY
    ds_plugin = DeepSpeedPlugin(hf_ds_config=ds_config_value)  # This is now a dict, not a string path
...
# ============================================================
    ...
    trainer = TorchTrainer(
        ...
        run_config=train.RunConfig(
            ...
            storage_path=args.storage_path,  # SIMPLE: Use storage_path directly
